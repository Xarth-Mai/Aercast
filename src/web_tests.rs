...
async fn token_routes_wait_between_isolated_media_sessions() {
    ...
    for required in [
        ...
        b"if (end - start < START_BUFFER) continue;".as_slice(),
        b"else if (!automaticPlay && !video.paused)".as_slice(),
        b"if (!video.seeking) seekTo(Math.max(start, end - LIVE_EDGE_DELAY))".as_slice(),
        b"else if (lag > 1.5)".as_slice(),
        b"video.playbackRate = 1.0 + Math.min(0.15, lag * 0.08)".as_slice(),
        b"seekTo(Math.max(start, end - LIVE_EDGE_DELAY))".as_slice(),
