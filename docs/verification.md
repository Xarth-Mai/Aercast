...
The user reported Windows playback as normal with approximately 1300 ms lag, while iOS lag fluctuated around 2000–3000 ms and above with intermittent stutter. After trying a newer version, the user reported substantially improved iOS playback with occasional stutter remaining. The exact running revision, browser/OS builds, quality settings, network path, and observation duration were not captured; this is user-reported behavior, not a controlled comparison or platform qualification. It does not isolate catch-up speed, automatic seeking, decoding, or network delivery as the cause

The pending tuning uses one shared policy for all platforms, as defined in the [Viewer contract](development.md#viewer-management-and-playback); separate desktop/mobile settings and a normal-speed-only comparison were discussed but not implemented. The user feedback above predates this pending tuning and does not validate it. Telemetry measures the last buffered end minus playback time, not end-to-end latency or necessarily contiguous playable buffer ahead; a multi-second lag does not rule out a buffer gap near playback. Buffer shortage remains unconfirmed without correlating a stutter with local buffered ranges, seeking, and frame progress

On 2026-09-12, `bun test ./tests/viewer-recovery.test.js` passed 13 tests with 251 assertions and `git diff --check` passed. The shipped Viewer script was exercised with browser API doubles to verify normal speed through 2 seconds of lag, continuous acceleration above 2 seconds capped at 1.05× from 2.625 seconds, a 1.75-second automatic-correction target, no automatic seek at 5 seconds of lag, transient lag rejection, sustained-lag correction above 5 seconds, a 10-second correction cooldown, normal-speed recovery, and timer reset during pause, seeking, and hidden-page appends. The new acceleration curve, correction target, 3-second persistence, and 10-second cooldown values have not been compared on a real iPhone; intermittent iOS stutter remains unresolved at the device-validation level. These checks do not establish decoder behavior, smooth playback, or end-to-end latency
