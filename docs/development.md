...
Aercast v0.1.6 is an early x86-64 Linux release distributed through AUR, GitHub
...
- All platforms share the same playback tuning. Playback starts after 1.75 seconds are buffered and targets 1.75 seconds behind the live edge for initial positioning, automatic correction, and manual seeking. Lag up to 2 seconds uses normal speed; above 2 seconds, playback speed increases continuously by 0.08× per additional second of lag, reaching its 1.05× cap at 2.625 seconds. Automatic correction requires lag above 5 seconds across append observations spanning at least 3 seconds, and at least 10 seconds since initial positioning or the previous automatic correction. Lag returning to 5 seconds or less, or an append observed during pause, autoplay startup, seeking, or a hidden page resets the sustained-lag timer. Those inactive states use normal speed and do not automatically seek. Manual seeking still returns to the live position. The correction timing is a provisional stability policy pending real iPhone comparison
...
- Video retains 8-bit 4:2:0 and fixes both encoders to H.264 High with B frames disabled and encoder-selected Level. Profile is not configurable; x264 enables CABAC and 8×8 transforms. High requires target-browser validation. VA-API uses balanced `target-usage=4`; x264 uses `superfast` with `zerolatency`. These settings favor compression quality over the previous fastest presets while retaining the existing bitrate, one-second maximum keyframe interval, and buffer limits; actual quality and encoding cost depend on content and hardware
